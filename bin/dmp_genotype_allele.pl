...
sub RunMpileup
{
	...
	`$QSUB -q $queue -V -wd $outdir -hold_jid RunPileup.$bamId.$$ -N NotifyMpileup.$bamId.$$ -e /dev/null -o NotifyMpileup.$bamId.$$.stat -l h_vmem=2G,virtual_free=2G -pe smp 1 -b y "$outdir/Notify.csh"`;
           	...
           	{
           		my $cmd = "$BSUB -q $queue -J RunPileup.$bamId.$$ -cwd $outdir -e RunPileup.$bamId.$$.%J.stderr -o RunPileup.$bamId.$$.%J.stdout -We 24:00 -R \"rusage[mem=5]\" -M 8 -n 1 \"$pathToSamtools $command\"";
           		$logger->debug($cmd);
           		`$BSUB -q $queue -J RunPileup.$bamId.$$ -cwd $outdir -e RunPileup.$bamId.$$.%J.stderr -o RunPileup.$bamId.$$.%J.stdout -We 24:00 -R "rusage[mem=5]" -M 8 -n 1 "$pathToSamtools $command"`;
