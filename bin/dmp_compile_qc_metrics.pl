...
sub PlotGraphs
{
    ...
    {
    	my $cmd = "$BSUB -q $queue -J PCW.$$ -cwd $outdir -e PCW.$$.%J.stderr -o PCW.$$.%J.stdout -We 0:59 -R \"rusage[mem=5]\" -M 10 -n 1 \"$outdir/PostCompileWrapper.csh\"";
    	$logger->debug( "COMMAND: $cmd");
    	eval{
    		`$BSUB -q $queue -J PCW.$$ -cwd $outdir -e PCW.$$.%J.stderr -o PCW.$$.%J.stdout -We 0:59 -R "rusage[mem=5]" -M 10 -n 1 "$outdir/PostCompileWrapper.csh"`;
			`$BSUB -q $queue -cwd $outdir -w "post_done(PCW.$$)" -J NotifyPCW.$$ -e NotifyPCW.$$.%J.stderr -o NotifyPCW.$$.stat -We 0:59 -R "rusage[mem=2]" -M 4 -n 1 "$outdir/Notify.csh"`;	
 ...
 eval{
    ...
    if ($@) {
    	...
    	eval{
			...
			`$BSUB -q $queue -cwd $outdir -w "post_done(PreparePDF.$$)" -J NotifyPreparePDF.$$ -e NotifyPreparePDF.$$.%J.stderr -o NotifyPreparePDF.$$.stat -We 0:59 -R "rusage[mem=2]" -M 4 -n 1 "$outdir/Notify.csh"`;	
