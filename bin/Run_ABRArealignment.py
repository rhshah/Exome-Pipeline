...
def main():
    parser = argparse.ArgumentParser(
        prog='Run_AbraRealignment.py',
        description='Run ABRA Indel Realignment',
    ...
    parser.add_argument(
        ...
        dest="bamList",
        required=True,
        metavar='BamFile.list',
        help="Full path to the tumor bam files as a fof.")
    parser.add_argument(
        "-p",
        "--patientId",
        action="store",
        ...
        required=True,
        metavar='PatientID',
        help="Id of the Patient for which the bam files are to be realigned")
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
    ...
    parser.add_argument(
        ...
        required=True,
        metavar='5',
        help="Number of Threads to be used to run ABRA")
    parser.add_argument(
        "-d",
        "--mdp",
        action="store_true",
    ...
    parser.add_argument(
        ...
        required=True,
        metavar='43',
        help="Number of k-mers to be used to run ABRA; Multiple k-mers are separated by space")
    parser.add_argument(
        "-temp",
        "--temporaryDirectory",
        action="store",
        dest="tmpDir",
        required=True,
        metavar='/somepath/tmpdir',
        help="Full Path to temporary directory")
    parser.add_argument(
        "-r", "--referenceFile", action="store", dest="ref", required=True,
    ...
    parser.add_argument(
        ...
        dest="ABRA",
        required=True,
        metavar='/somepath/ABRA.jar',
        help="Full Path to the ABRA jar file.")
    parser.add_argument(
        "-tr",
        "--targetRegion",
        action="store",
        dest="targetRegion",
        required=True,
        metavar='/somepath/targetRegion.bed',
        help="Full Path to the target region bed file")
    parser.add_argument(
        "-j",
        "--javaPATH",
        action="store",
    ...
    parser.add_argument(
        ...
        dest="BWA",
        required=True,
        metavar='/somepath/bin',
        help="Path to the bin of bwa executable.")
    parser.add_argument(
        "-q",
        "--queue",
        action="store",
...
def RunABRA(args, inBamList, outBamList, kmers, tmpdir):
    ...
    cmd = args.JAVA + " -Xmx80g -jar " + args.ABRA + " -mnf 5 --mbq 150 --in " + inBamList + " --out " + outBamList + " --ref " + args.ref + " --targets " + \
        args.targetRegion + " --threads " + args.threads + " --mad " + str(args.dp) + " --kmer " + kmers + " --working " + tmpdir
    # print "CMD==>",cmd,"\n"
    os.environ["PATH"] = os.environ["PATH"] + ":" + args.BWA
    cl_cmd = ''
    mem = int(args.threads) * 7
    maxmem = int(mem) + 5
    if(args.qsub):
        cl_cmd = args.qsub + " -q " + args.queue + " -N " + "ABRA_" + args.patientId + "_" + str(myPid) + " -o " + "ABRA_" + args.patientId + "_" + str(
            myPid) + ".stdout" + " -e " + "ABRA_" + args.patientId + "_" + str(myPid) + ".stderr" + " -V -l h_vmem=5G,virtual_free=5G -pe smp " + args.threads + " -wd " + args.outdir + " -sync y " + " -b y " + cmd
    else:
        cl_cmd = args.bsub + " -q " + args.queue + " -J " + "ABRA_" + args.patientId + "_" + str(myPid) + " -o " + "ABRA_" + args.patientId + "_" + str(myPid) + ".stdout" + " -e " + "ABRA_" + args.patientId + "_" + str(
            myPid) + ".stderr" + " -We 48:00 -R \"rusage[mem=" + str(mem) + "]\" -M " + str(maxmem) + " -n " + args.threads + " -cwd " + args.outdir + " -K " + cmd
